...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="p[@contentType=$sourceLang]">
        ...
        <xsl:element name="p">
            <xsl:attribute name="contentType"><xsl:value-of select="$targetLang"/></xsl:attribute>
            <xsl:apply-templates select="node()"/>
        </xsl:element>
    </xsl:template>
    <xsl:template match="p[not(@contentType)]">
            <xsl:copy>
                <xsl:apply-templates select="@* | node()"/>
            </xsl:copy>
        <xsl:element name="p">
        ...
        <xsl:copy>
            ...
            <xsl:element name="labelContentChoices">
                <xsl:element name="labelContent">
                    <xsl:attribute name="contentType"><xsl:value-of select="$sourceLang"/></xsl:attribute>
